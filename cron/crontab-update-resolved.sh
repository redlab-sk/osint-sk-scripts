...
echo "$(date '+%Y-%m-%d %H:%M:%S,000') - crontab-update-resolved - INFO - BEGIN." >> ${LOGDIR}/crontab.log
echo "$(date '+%Y-%m-%d %H:%M:%S,000') - crontab-update-resolved - INFO - Executing update-resolved." >> ${LOGDIR}/crontab.log

# Execute update script (list/id/path)
RESULT=`${BINDIR}/update-resolved.py -au`
# Is there a message?
if [ -n "$RESULT" ]
then
...
echo "$(date '+%Y-%m-%d %H:%M:%S,000') - crontab-update-resolved - INFO - Setting ssh environment variables from keychain." >> ${LOGDIR}/crontab.log
. ${KEYCHAIN}

# git commit the changes
cd ${GITDIR}
echo "$(date '+%Y-%m-%d %H:%M:%S,000') - crontab-update-resolved - INFO - Executing git coommit + push." >> ${LOGDIR}/crontab.log
...
# Just let me know
echo "$(date '+%Y-%m-%d %H:%M:%S,000') - crontab-update-resolved - INFO - Sending notification." >> ${LOGDIR}/crontab.log
echo -e "update-resolved: ${RESULT_UPDATE}\ngit-commit: ${RESULT_GIT}\n\n${OUTPUT}" | /usr/local/bin/signal-cli -u "${SIGNAL_SRC}" send "${SIGNAL_DST}"
echo "$(date '+%Y-%m-%d %H:%M:%S,000') - crontab-update-resolved - INFO - END." >> ${LOGDIR}/crontab.log
